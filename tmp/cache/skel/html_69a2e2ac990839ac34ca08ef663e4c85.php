...
/*
 * Squelette : ../plugins/auto/saisies/v3.33.0/saisies/selection.html
 * Date :      Fri, 20 Dec 2019 11:26:28 GMT
 * Compile :   Fri, 21 Feb 2020 08:30:41 GMT
 * Boucles :   _optgroup, _cond, _selection
 */ 

function BOUCLE_optgrouphtml_69a2e2ac990839ac34ca08ef663e4c85(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"DATA",
		$command,
		array('../plugins/auto/saisies/v3.33.0/saisies/selection.html','html_69a2e2ac990839ac34ca08ef663e4c85','_optgroup',51,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
'</option>
	...
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_optgroup @ ../plugins/auto/saisies/v3.33.0/saisies/selection.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_condhtml_69a2e2ac990839ac34ca08ef663e4c85(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"CONDITION",
		$command,
		array('../plugins/auto/saisies/v3.33.0/saisies/selection.html','html_69a2e2ac990839ac34ca08ef663e4c85','_cond',49,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
	while ($Pile[$SP]=$iter->fetch()) {
		...
		$t0 .= BOUCLE_optgrouphtml_69a2e2ac990839ac34ca08ef663e4c85($Cache, $Pile, $doublons, $Numrows, $SP);
	}
	$iter->free();
	}
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_cond @ ../plugins/auto/saisies/v3.33.0/saisies/selection.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_selectionhtml_69a2e2ac990839ac34ca08ef663e4c85(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"DATA",
		$command,
		array('../plugins/auto/saisies/v3.33.0/saisies/selection.html','html_69a2e2ac990839ac34ca08ef663e4c85','_selection',48,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
'
	' .
(($t1 = BOUCLE_condhtml_69a2e2ac990839ac34ca08ef663e4c85($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
	<optgroup label="' .
		interdire_scripts($Pile[$SP]['cle']) .
		'">
...
');
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_selection @ ../plugins/auto/saisies/v3.33.0/saisies/selection.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}

//
// Fonction principale du squelette ../plugins/auto/saisies/v3.33.0/saisies/selection.html
// Temps de compilation total: 4.453 ms
//

function html_69a2e2ac990839ac34ca08ef663e4c85($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
...
'
' .
BOUCLE_selectionhtml_69a2e2ac990839ac34ca08ef663e4c85($Cache, $Pile, $doublons, $Numrows, $SP) .
'
</select>
');

	return analyse_resultat_skel('html_69a2e2ac990839ac34ca08ef663e4c85', $Cache, $page, '../plugins/auto/saisies/v3.33.0/saisies/selection.html');
}
?>