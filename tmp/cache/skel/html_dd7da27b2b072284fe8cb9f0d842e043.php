 ...
 * Compile :   Thu, 27 Feb 2020 09:02:24 GMT
...
(($t1 = strval(invalideur_session($Cache, ((((function_exists("autoriser")||include_spip("inc/autoriser"))&&autoriser('webmestre')?" ":"")) ?' ' :''))))!=='' ?
	...
	(($t2 = strval(((($a = abs(version_svn_courante(concat((defined(interdire_scripts($Pile[$SP]['constante']))?constant(interdire_scripts($Pile[$SP]['constante'])):''),(	interdire_scripts($Pile[$SP]['src_archive']) .
	...
	'</dt>
		<dd>' .
	joli_repertoire(concat((defined(interdire_scripts($Pile[$SP]['constante']))?constant(interdire_scripts($Pile[$SP]['constante'])):''),(	interdire_scripts($Pile[$SP]['src_archive']) .
		'/'))) .
	'</dd>
		' .
...
// Temps de compilation total: 9.117 ms
//
