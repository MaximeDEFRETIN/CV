 ...
 * Compile :   Thu, 06 Feb 2020 16:16:49 GMT
...
function BOUCLE_liste_authtml_f0b6d1d9e46089b46cc88aefee56febc(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  'lien' => true,
	...
	$command['where'] = 
			array((!(is_array(@$Pile[0]['id_auteur'])?count(@$Pile[0]['id_auteur']):strlen(@$Pile[0]['id_auteur'])) ? '' : ((is_array(@$Pile[0]['id_auteur'])) ? sql_in('auteurs.id_auteur',sql_quote($in)) : 
			array('=', 'auteurs.id_auteur', sql_quote(@$Pile[0]['id_auteur'], '','bigint(21) NOT NULL AUTO_INCREMENT')))), (!(is_array(@$Pile[0]['id_mot'])?count(@$Pile[0]['id_mot']):strlen(@$Pile[0]['id_mot'])) ? '' : ((is_array(@$Pile[0]['id_mot'])) ? sql_in('L1.id_mot',sql_quote($in1)) : 
			array('=', 'L1.id_mot', sql_quote(@$Pile[0]['id_mot'], '','bigint(21) NOT NULL DEFAULT 0')))), ((@$Pile[0]["where"]) ? (@$Pile[0]["where"]) : ''), (!(is_array(@$Pile[0]['statut'])?count(@$Pile[0]['statut']):strlen(@$Pile[0]['statut'])) ? '' : ((is_array(@$Pile[0]['statut'])) ? sql_in('auteurs.statut',sql_quote($in2)) : 
			array('=', 'auteurs.statut', sql_quote(@$Pile[0]['statut'], '','varchar(255) NOT NULL DEFAULT \'0\'')))), $rech_where?$rech_where:'');
	...
	$l4 = _T('public|spip|ecrire:statut_admin_restreint');
	$l5 = _T('public|spip|ecrire:admin_modifier_auteur');$SP++;
	// RESULTATS
	while ($Pile[$SP]=$iter->fetch()) {
...
// Temps de compilation total: 9.477 ms
//
