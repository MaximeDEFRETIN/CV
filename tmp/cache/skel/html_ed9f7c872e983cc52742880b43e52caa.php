...
function BOUCLE_decomptehtml_ed9f7c872e983cc52742880b43e52caa(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			...
			array('REGEXP', 'forum.statut', sql_quote(table_valeur($Pile["vars"], (string)'prive', null), '', 'char')));
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_ed9f7c872e983cc52742880b43e52caa','_decompte',7,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	...
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_decompte @ ../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_dochtml_ed9f7c872e983cc52742880b43e52caa(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('documents.statut','publie,prop,prepa','publie',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_ed9f7c872e983cc52742880b43e52caa','_doc',34,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
interdire_scripts((match($Pile[$SP]['extension'],'^(gif|jpg|png)$') ? (	filtrer('image_graver',filtrer('image_reduire',
	...
	'recurs'=>(++$recurs)), array('compil'=>array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_ed9f7c872e983cc52742880b43e52caa','_doc',36,$GLOBALS['spip_lang']), 'trim'=>true), ''))
,'300')) .
	'
...
'
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_doc @ ../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_doc2html_ed9f7c872e983cc52742880b43e52caa(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('documents.statut','publie,prop,prepa','publie',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_ed9f7c872e983cc52742880b43e52caa','_doc2',69,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
interdire_scripts((match($Pile[$SP]['extension'],'^(gif|jpg|png)$') ? (	filtrer('image_graver',filtrer('image_reduire',
	...
	'id_document'=>$Pile[$SP]['id_document'],
	'id'=>$Pile[$SP]['id_document'],
	'recurs'=>(++$recurs)), array('compil'=>array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_ed9f7c872e983cc52742880b43e52caa','_doc2',71,$GLOBALS['spip_lang']), 'trim'=>true), ''))
,'300')) .
	'
				'):(	quete_logo_document(quete_document($Pile[$SP]['id_document'], ''), vider_url(urlencode_1738(generer_url_entite($Pile[$SP]['id_document'], 'document', '', '', true))), '', '', 0, 0, '') .
	'
...
'
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_doc2 @ ../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_forums_bouclehtml_ed9f7c872e983cc52742880b43e52caa(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {

	static $command = array();
	static $connect;
	$command['connect'] = $connect = '';
	$save_numrows = (isset($Numrows['_forums_fils']) ? $Numrows['_forums_fils'] : array());
	$t0 = (($t1 = BOUCLE_forums_filshtml_ed9f7c872e983cc52742880b43e52caa($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		('
		<ul>
			' . $t1 . '
		</ul>
...
function BOUCLE_forums_filshtml_ed9f7c872e983cc52742880b43e52caa(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_ed9f7c872e983cc52742880b43e52caa','_forums_fils',48,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
(($Pile[$SP]['id_auteur'])  ?
	...
	'id'=>$Pile[$SP]['id_forum'],
	'recurs'=>(++$recurs)), array('compil'=>array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_ed9f7c872e983cc52742880b43e52caa','_forums_fils',53,$GLOBALS['spip_lang']), 'trim'=>true), ''))
 .
	'</a>
						')) :
		'') .
(($t1 = strval(($Pile[$SP]['id_auteur'] ? '':interdire_scripts(liens_nofollow(safehtml(vider_url($Pile[$SP]['auteur'])))))))!=='' ?
		($t1 . '
						') :
		'') .
(!($Pile[$SP]['id_auteur'])  ?
...
'
				...
				' .
BOUCLE_doc2html_ed9f7c872e983cc52742880b43e52caa($Cache, $Pile, $doublons, $Numrows, $SP) .
'


					' .
(($t1 = strval(invalideur_session($Cache, ((((function_exists("autoriser")||include_spip("inc/autoriser"))&&autoriser('participerforumprive', interdire_scripts(invalideur_session($Cache, $Pile[$SP]['objet'])), invalideur_session($Cache, $Pile[$SP]['id_objet']))?" ":"")) ?' ' :''))))!=='' ?
		($t1 . (	'
						' .
	(($t2 = strval(parametre_url(self(),'repondre',$Pile[$SP]['id_forum'])))!=='' ?
			('<p class="repondre"><a class=\'ajax\' href="' . $t2 . (	'" rel="noindex nofollow">' .
		$l3 .
		'</a></p>')) :
			'') .
	'
...
'
				...
				' .
BOUCLE_forums_bouclehtml_ed9f7c872e983cc52742880b43e52caa($Cache, $Pile, $doublons, $Numrows, $SP) .
'
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_forums_fils @ ../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_forumshtml_ed9f7c872e983cc52742880b43e52caa(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_ed9f7c872e983cc52742880b43e52caa','_forums',10,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
(($Pile[$SP]['id_auteur'])  ?
	...
	'id_forum'=>$Pile[$SP]['id_forum'],
	'id'=>$Pile[$SP]['id_forum'],
	'recurs'=>(++$recurs)), array('compil'=>array('../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html','html_ed9f7c872e983cc52742880b43e52caa','_forums',17,$GLOBALS['spip_lang']), 'trim'=>true), ''))
 .
	'</a>
				')) :
		'') .
...
'
				...
				' .
BOUCLE_dochtml_ed9f7c872e983cc52742880b43e52caa($Cache, $Pile, $doublons, $Numrows, $SP) .
'
...
'
		...
		' .
(($t1 = BOUCLE_forums_filshtml_ed9f7c872e983cc52742880b43e52caa($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		('
		<ul>
			' . $t1 . '
		</ul>
...
'
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_forums @ ../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}

//
// Fonction principale du squelette ../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html
// Temps de compilation total: 11.682 ms
//

function html_ed9f7c872e983cc52742880b43e52caa($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
...
'
' .
vide($Pile['vars'][$_zzz=(string)'prive'] = interdire_scripts(((entites_html(sinon(table_valeur(@$Pile[0], (string)'quoi', null), 'public'),true) == 'interne') ? '^(priv|perso)':'.*'))) .
'
' .
BOUCLE_decomptehtml_ed9f7c872e983cc52742880b43e52caa($Cache, $Pile, $doublons, $Numrows, $SP)
...
' .
(($t1 = BOUCLE_forumshtml_ed9f7c872e983cc52742880b43e52caa($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
' .
...
');
	...
	return analyse_resultat_skel('html_ed9f7c872e983cc52742880b43e52caa', $Cache, $page, '../plugins-dist/forum/prive/objets/liste/participer_forum_thread.html');
}
?>