 ...
 * Date :      Sun, 22 Mar 2020 15:55:57 GMT
 * Compile :   Sun, 22 Mar 2020 16:27:11 GMT
 * Boucles :   _lettre, _liste_aut
 */ 

function BOUCLE_lettrehtml_c1d158ae53b9d3e3d072ff0ef94a68af(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  array (
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../prive/objets/liste/visiteurs.html','html_c1d158ae53b9d3e3d072ff0ef94a68af','_lettre',18,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
function BOUCLE_liste_authtml_c1d158ae53b9d3e3d072ff0ef94a68af(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  array (
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../prive/objets/liste/visiteurs.html','html_c1d158ae53b9d3e3d072ff0ef94a68af','_liste_aut',7,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
(($t1 = strval(recuperer_numero($Pile[$SP]['titre_rang'])))!=='' ?
		($t1 . '. ') :
		'') .
interdire_scripts(((($a = ((($a = trim(supprimer_numero(typo($Pile[$SP]['nom']), "TYPO", $connect, $Pile[0]))) OR (is_string($a) AND strlen($a))) ? $a : interdire_scripts(trim(couper($Pile[$SP]['bio'],'80'))))) OR (is_string($a) AND strlen($a))) ? $a : (	$l2 .
	$Pile[$SP]['id_auteur']))) .
'</a></td>
			' .
...
interdire_scripts(((entites_html(table_valeur(@$Pile[0], (string)'statut', null),true) == 'nouveau') ? interdire_scripts(affdate($Pile[$SP]['maj'])):(	(($t2 = strval(singulier_ou_pluriel($Pile[$SP]['compteur_articles'],'info_1_article','info_nb_articles')))!=='' ?
			($t2 . '<br />') :
			'') .
...
// Temps de compilation total: 9.717 ms
//

function html_c1d158ae53b9d3e3d072ff0ef94a68af($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
...
') :
		'') .
(($t1 = BOUCLE_liste_authtml_c1d158ae53b9d3e3d072ff0ef94a68af($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
' .
...
' .
	...
	<thead>
		...
		(($t3 = BOUCLE_lettrehtml_c1d158ae53b9d3e3d072ff0ef94a68af($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		...
		interdire_scripts(((entites_html(table_valeur(@$Pile[0], (string)'statut', null),true) == 'nouveau') ? _T('public|spip|ecrire:info_inscription'):_T('public|spip|ecrire:info_contributions'))) .
		'</th>
		</tr>
	</thead>
	<tbody>
...
');
	...
	return analyse_resultat_skel('html_c1d158ae53b9d3e3d072ff0ef94a68af', $Cache, $page, '../prive/objets/liste/visiteurs.html');
}
?>