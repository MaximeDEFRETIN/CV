...
/*
 * Squelette : ../plugins/auto/escal_V4/prive/objets/liste/articles.html
 * Date :      Fri, 20 Dec 2019 11:26:32 GMT
 * Compile :   Thu, 27 Feb 2020 09:07:29 GMT
 * Boucles :   _mots, _auteurs, _liste_art
 */ 

function BOUCLE_motshtml_c9ab52373d516a688ba07c4be1462470(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	if (!isset($command['table'])) {
		...
		"mots.titre");
		$command['orderby'] = array('mots.id_groupe');
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins/auto/escal_V4/prive/objets/liste/articles.html','html_c9ab52373d516a688ba07c4be1462470','_mots',30,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
	...
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_mots @ ../plugins/auto/escal_V4/prive/objets/liste/articles.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_auteurshtml_c9ab52373d516a688ba07c4be1462470(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('auteurs.statut','!5poubelle','!5poubelle',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins/auto/escal_V4/prive/objets/liste/articles.html','html_c9ab52373d516a688ba07c4be1462470','_auteurs',37,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
'</a>');
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_auteurs @ ../plugins/auto/escal_V4/prive/objets/liste/articles.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_liste_arthtml_c9ab52373d516a688ba07c4be1462470(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  'lien' => true,
	...
	$senstri = '';
	$tri = (($t=(isset($Pile[0]['tri'.'_liste_art']))?$Pile[0]['tri'.'_liste_art']:((strncmp('_liste_art','session',7)==0 AND session_get('tri'.'_liste_art'))?session_get('tri'.'_liste_art'):interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'par', null), 'date'),true))))?tri_protege_champ($t):'');
	if ($tri){
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins/auto/escal_V4/prive/objets/liste/articles.html','html_c9ab52373d516a688ba07c4be1462470','_liste_art',7,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
' ' .
(($t1 = strval(interdire_scripts((((entites_html(table_valeur(@$Pile[0], (string)'lang', null),true) == spip_htmlentities($Pile[$SP]['lang'] ? $Pile[$SP]['lang'] : $GLOBALS['spip_lang']))) ?'' :' '))))!=='' ?
		($t1 . (	' &#40;' .
...
'
					' .
(($t1 = BOUCLE_motshtml_c9ab52373d516a688ba07c4be1462470($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
					<small><br /><img src="' .
		interdire_scripts(chemin_image('mot-16.png')) .
		'" />
					') . $t1 . '
					</small>
					') :
...
BOUCLE_auteurshtml_c9ab52373d516a688ba07c4be1462470($Cache, $Pile, $doublons, $Numrows, $SP) .
'</div></td>
...
'</td>
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_liste_art @ ../plugins/auto/escal_V4/prive/objets/liste/articles.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}

//
// Fonction principale du squelette ../plugins/auto/escal_V4/prive/objets/liste/articles.html
// Temps de compilation total: 35.879 ms
//

function html_c9ab52373d516a688ba07c4be1462470($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
	...
	$connect = '';
	$page = (
(($t1 = strval(vide($Pile['vars'][$_zzz=(string)'defaut_tri'] = array('date' => interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'date_sens', null), '-1'),true)), 'num titre' => '1', 'id_article' => '1', 'points' => '-1
'))))!=='' ?
		($t1 . '
') :
		'') .
(($t1 = BOUCLE_liste_arthtml_c9ab52373d516a688ba07c4be1462470($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
' .
...
') :
	...
	return analyse_resultat_skel('html_c9ab52373d516a688ba07c4be1462470', $Cache, $page, '../plugins/auto/escal_V4/prive/objets/liste/articles.html');
}
?>