...
function BOUCLE_comptemhtml_089d2be028cac8ea7d2b669f8713cac8(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  array (
	...
	$command['where'] = 
			...
			array('=', 'documents.media', sql_quote(interdire_scripts(safehtml($Pile[$SP]['valeur'])), '', 'varchar(10) NOT NULL DEFAULT \'file\'')));
...
function BOUCLE_comptehtml_089d2be028cac8ea7d2b669f8713cac8(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  array (
	...
	$command['where'] = 
			...
			array('=', 'documents.media', sql_quote(interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'media', null),true)), '', 'varchar(10) NOT NULL DEFAULT \'file\'')));
	if (defined("_BOUCLE_PROFILER")) $timer = time()+(float)microtime();
	$t0 = "";
	// REQUETE
	$iter = IterFactory::create(
...
// Temps de compilation total: 34.770 ms
//
