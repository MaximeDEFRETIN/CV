...
/*
 * Squelette : ../plugins/auto/saisies/v3.31.3/saisies/_base.html
 * Date :      Fri, 20 Dec 2019 11:26:28 GMT
 * Compile :   Wed, 05 Feb 2020 16:19:35 GMT
 * Boucles :   
 */ 
//
// Fonction principale du squelette ../plugins/auto/saisies/v3.31.3/saisies/_base.html
// Temps de compilation total: 4.661 ms
//

function html_c9269e770fe83876664ef1f1a3bcc9b5($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
...
(($t1 = strval(interdire_scripts(((entites_html(table_valeur(@$Pile[0], (string)'nom', null),true)) ?' ' :''))))!=='' ?
	...
	'readonly' => table_valeur($Pile["vars"], (string)'readonly', null) )), array('compil'=>array('../plugins/auto/saisies/v3.31.3/saisies/_base.html','html_c9269e770fe83876664ef1f1a3bcc9b5','',12,$GLOBALS['spip_lang'])), _request('connect')) .
		'
	')) :
			'') .
	'
    ...
    ' .
		...
		vide($Pile['vars'][$_zzz=(string)'erreurs'] = sinon(table_valeur($Pile["vars"], (string)'erreurs', null), table_valeur(@$Pile[0], (string)(	'erreurs/' .
		...
		(($t3 = strval(interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'conteneur_class', null), interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'li_class', null),true))),true))))!=='' ?
				(' ' . $t3) :
				'') .
		(($t3 = strval(interdire_scripts(saisie_type2classe(entites_html(table_valeur(@$Pile[0], (string)'type_saisie', null),true)))))!=='' ?
				(' ' . $t3) :
				'') .
		'"' .
		(($t3 = strval(interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'id_saisie', null),true))))!=='' ?
				(' data-id="' . $t3 . '"') :
				'') .
		(($t3 = strval(interdire_scripts(saisies_afficher_si_js(table_valeur(@$Pile[0], (string)'afficher_si', null),interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'_saisies', null),true))))))!=='' ?
				(' data-afficher_si="' . $t3 . '"') :
				'') .
		'>
			' .
...
');
	...
	return analyse_resultat_skel('html_c9269e770fe83876664ef1f1a3bcc9b5', $Cache, $page, '../plugins/auto/saisies/v3.31.3/saisies/_base.html');
}
?>