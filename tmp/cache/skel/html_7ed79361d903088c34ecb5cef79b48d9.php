...
quete_condition_statut('documents.statut','publie,prop,prepa','publie',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/inclure/documents.html','html_7ed79361d903088c34ecb5cef79b48d9','_documents_portfolio',6,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
quete_condition_statut('documents.statut','publie,prop,prepa','publie',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/inclure/documents.html','html_7ed79361d903088c34ecb5cef79b48d9','_afficher_document',29,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
filtrer('image_graver',filtrer('image_reduire',
	...
	'recurs'=>(++$recurs)), array('compil'=>array('squelettes/inclure/documents.html','html_7ed79361d903088c34ecb5cef79b48d9','_afficher_document',30,$GLOBALS['spip_lang']), 'trim'=>true), ''))
,'672','*')) .
'
');
	}
...
quete_condition_statut('documents.statut','publie,prop,prepa','publie',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/inclure/documents.html','html_7ed79361d903088c34ecb5cef79b48d9','_documents_joints',46,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
quete_condition_statut('documents.statut','publie,prop,prepa','publie',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/inclure/documents.html','html_7ed79361d903088c34ecb5cef79b48d9','_documents_decompte',37,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'
' .
(($Numrows['_documents_decompte']['total'] == '1') ? trim(recuperer_fond( 'modeles/emb' , array('id_document' => $Pile[$SP]['id_document'] ), array('compil'=>array('squelettes/inclure/documents.html','html_7ed79361d903088c34ecb5cef79b48d9','_documents_decompte',39,$GLOBALS['spip_lang'])), _request('connect'))):''));
	}
	$iter->free();
	}
	if (defined("_BOUCLE_PROFILER")
