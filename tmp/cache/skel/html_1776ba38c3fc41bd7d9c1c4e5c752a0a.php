...
function BOUCLE_mots_rsshtml_1776ba38c3fc41bd7d9c1c4e5c752a0a(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/inc-rss-item.html','html_1776ba38c3fc41bd7d9c1c4e5c752a0a','_mots_rss',14,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
function BOUCLE_rubrique_mfhtml_1776ba38c3fc41bd7d9c1c4e5c752a0a(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('rubriques.statut','!','publie',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/inc-rss-item.html','html_1776ba38c3fc41bd7d9c1c4e5c752a0a','_rubrique_mf',23,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
function BOUCLE_mots_mfhtml_1776ba38c3fc41bd7d9c1c4e5c752a0a(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/inc-rss-item.html','html_1776ba38c3fc41bd7d9c1c4e5c752a0a','_mots_mf',25,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
function BOUCLE_documentshtml_1776ba38c3fc41bd7d9c1c4e5c752a0a(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('documents.statut','publie,prop,prepa','publie',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/inc-rss-item.html','html_1776ba38c3fc41bd7d9c1c4e5c752a0a','_documents',50,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
function BOUCLE_un_articlehtml_1776ba38c3fc41bd7d9c1c4e5c752a0a(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('articles.statut','publie,prop,prepa/auteur','publie',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/inc-rss-item.html','html_1776ba38c3fc41bd7d9c1c4e5c752a0a','_un_article',1,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	lang_select($GLOBALS['spip_lang']);
	$SP++;
...
'</link>
		' .
(($t1 = strval(texte_backend(url_absolue(vider_url(urlencode_1738(generer_url_entite($Pile[$SP]['id_article'], 'article', '', '', true)))))))!=='' ?
...
'
		' .
(($t1 = strval(texte_backend(supprimer_tags(recuperer_fond('modeles/lesauteurs', array('objet'=>'article','id_objet' => $Pile[$SP]['id_article'],'id_article' => $Pile[$SP]['id_article']), array('trim'=>true, 'compil'=>array('squelettes/inc-rss-item.html','html_1776ba38c3fc41bd7d9c1c4e5c752a0a','_un_article',7,$GLOBALS['spip_lang'])), '')))))!=='' ?
		('<dc:creator>' . $t1 . '</dc:creator>') :
		'') .
'

' .
BOUCLE_mots_rsshtml_1776ba38c3fc41bd7d9c1c4e5c752a0a($Cache, $Pile, $doublons, $Numrows, $SP) .
'
...
' .
BOUCLE_rubrique_mfhtml_1776ba38c3fc41bd7d9c1c4e5c752a0a($Cache, $Pile, $doublons, $Numrows, $SP) .
(($t1 = BOUCLE_mots_mfhtml_1776ba38c3fc41bd7d9c1c4e5c752a0a($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		('
/ ' . $t1) :
		'') .
'
...
((!is_array($l = quete_logo('id_article', 'ON', $Pile[$SP]['id_article'],'', 0))) ? '':
 ("<img class=\"spip_logo spip_logo_right spip_logos\" alt=\"\" style=\"float:right\" src=\"$l[0]\"" . $l[2] .  ($l[1] ? " onmouseover=\"this.src='$l[1]'\" onmouseout=\"this.src='$l[0]'\"" : "") . ' />')),'150','150')))))!=='' ?
			($t2 . '
		') :
			'') .
...
'
		...
		' .
BOUCLE_documentshtml_1776ba38c3fc41bd7d9c1c4e5c752a0a($Cache, $Pile, $doublons, $Numrows, $SP) .
'
...
// Temps de compilation total: 3.850 ms
//

function html_1776ba38c3fc41bd7d9c1c4e5c752a0a($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
	...
	$connect = '';
	$page = (
BOUCLE_un_articlehtml_1776ba38c3fc41bd7d9c1c4e5c752a0a($Cache, $Pile, $doublons, $Numrows, $SP) .
'
');

	return analyse_resultat_skel('html_1776ba38c3fc41bd7d9c1c4e5c752a0a', $Cache, $page, 'squelettes/inc-rss-item.html');
}
?>