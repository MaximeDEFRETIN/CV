...
/*
 * Squelette : ../plugins/auto/pages/v1.4.0/prive/objets/liste/pages.html
 * Date :      Fri, 20 Dec 2019 11:26:26 GMT
 * Compile :   Sun, 08 Mar 2020 10:06:19 GMT
 * Boucles :   _auteurs, _liste_art
 */ 

function BOUCLE_auteurshtml_7abd91039bfb5e9bc65c79967ddf7311(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('auteurs.statut','!5poubelle','!5poubelle',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins/auto/pages/v1.4.0/prive/objets/liste/pages.html','html_7abd91039bfb5e9bc65c79967ddf7311','_auteurs',31,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
'</a>');
	...
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_auteurs @ ../plugins/auto/pages/v1.4.0/prive/objets/liste/pages.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_liste_arthtml_7abd91039bfb5e9bc65c79967ddf7311(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
  ...
  'lien' => true,
	...
	$command['where'] = 
			...
			array('=', 'articles.statut', sql_quote(@$Pile[0]['statut'], '','varchar(10) NOT NULL DEFAULT \'0\'')))), $rech_where?$rech_where:'', ((isset($Pile[0]['id_rubrique'])?(is_array($Pile[0]['id_rubrique'])?count($Pile[0]['id_rubrique']):strlen($Pile[0]['id_rubrique'])):'') ? '' : 'articles.id_rubrique>0'));
	if (defined("_BOUCLE_PROFILER")) $timer = time()+(float)microtime();
	$t0 = "";
	// REQUETE
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins/auto/pages/v1.4.0/prive/objets/liste/pages.html','html_7abd91039bfb5e9bc65c79967ddf7311','_liste_art',7,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
BOUCLE_auteurshtml_7abd91039bfb5e9bc65c79967ddf7311($Cache, $Pile, $doublons, $Numrows, $SP) .
'</div></td>
...
'</td>
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_liste_art @ ../plugins/auto/pages/v1.4.0/prive/objets/liste/pages.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}

//
// Fonction principale du squelette ../plugins/auto/pages/v1.4.0/prive/objets/liste/pages.html
// Temps de compilation total: 32.352 ms
//

function html_7abd91039bfb5e9bc65c79967ddf7311($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
...
') :
		'') .
(($t1 = BOUCLE_liste_arthtml_7abd91039bfb5e9bc65c79967ddf7311($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
' .
...
');
	...
	return analyse_resultat_skel('html_7abd91039bfb5e9bc65c79967ddf7311', $Cache, $page, '../plugins/auto/pages/v1.4.0/prive/objets/liste/pages.html');
}
?>