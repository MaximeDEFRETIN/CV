...
function BOUCLE_afficher_contenuhtml_862edbf02769a3aa365384896e299823(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../prive/objets/contenu/auteur.html','html_862edbf02769a3aa365384896e299823','_afficher_contenu',1,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'\' class=\'' .
'pgp\'>' .
(($t1 = strval(interdire_scripts(filtrer('image_graver',filtrer('image_reduire',echapper_html_suspect($Pile[$SP]['pgp']),'500','0')))))!=='' ?
		('<code>' . $t1 . '</code>') :
		'') .
'</div>
</div>
...
');
	...
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_afficher_contenu @ ../prive/objets/contenu/auteur.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}

//
// Fonction principale du squelette ../prive/objets/contenu/auteur.html
// Temps de compilation total: 4.357 ms
//

function html_862edbf02769a3aa365384896e299823($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {

	if (isset($Pile[0]["doublons"]) AND is_array($Pile[0]["doublons"]))
		$doublons = nettoyer_env_doublons($Pile[0]["doublons"]);

	$connect = '';
	$page = BOUCLE_afficher_contenuhtml_862edbf02769a3aa365384896e299823($Cache, $Pile, $doublons, $Numrows, $SP);

	return analyse_resultat_skel('html_862edbf02769a3aa365384896e299823', $Cache, $page, '../prive/objets/contenu/auteur.html');
}
?>