...
interdire_scripts(supprimer_numero(typo($Pile[$SP]['type']), "TYPO", $connect, $Pile[0])) .
'</td>
...
// Temps de compilation total: 7.666 ms
//
...
' .
		(($t3 = strval(interdire_scripts(sinon(table_valeur(@$Pile[0], (string)'titre', null), singulier_ou_pluriel((isset($Numrows['_liste_mot']['grand_total'])
			...
			interdire_scripts((($aider=charger_fonction('aide','inc',true))?$aider('artmots','../plugins-dist/mots/prive/objets/liste/mots_lies.html', $Pile[0]):'')) .
			'</strong></caption>')) :
				'') .
		'
	<thead>
...
' .
		(($t3 = strval(((((isset($Numrows['_liste_mot']['grand_total'])
			? $Numrows['_liste_mot']['grand_total'] : $Numrows['_liste_mot']['total']) > '3')) ?' ' :'')))!=='' ?
				($t3 . (	'<div class="action"><button type="submit" class="button link" name="supprimer_lien[mot-*-' .
			interdire_scripts(@$Pile[0]['objet']) .
			'-' .
			@$Pile[0]['id_objet'] .
			']" value="X">' .
