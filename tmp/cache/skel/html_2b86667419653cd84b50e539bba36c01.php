...
/*
 * Squelette : plugins/auto/metasplus_v2/inclure/metasplus/dist.html
...
function BOUCLE_auteurs_objethtml_2b86667419653cd84b50e539bba36c01(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('auteurs.statut','!5poubelle','!5poubelle',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('plugins/auto/metasplus_v2/inclure/metasplus/dist.html','html_2b86667419653cd84b50e539bba36c01','_auteurs_objet',102,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
vide($Pile['vars'][$_zzz=(string)'auteurs_auto'] = filtre_push(table_valeur($Pile["vars"], (string)'auteurs_auto', null),interdire_scripts(supprimer_numero(typo($Pile[$SP]['nom']), "TYPO", $connect, $Pile[0])))));
	...
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_auteurs_objet @ plugins/auto/metasplus_v2/inclure/metasplus/dist.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_mots_objethtml_2b86667419653cd84b50e539bba36c01(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('plugins/auto/metasplus_v2/inclure/metasplus/dist.html','html_2b86667419653cd84b50e539bba36c01','_mots_objet',116,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
vide($Pile['vars'][$_zzz=(string)'mots_auto'] = filtre_push(table_valeur($Pile["vars"], (string)'mots_auto', null),interdire_scripts(supprimer_numero(typo($Pile[$SP]['titre']), "TYPO", $connect, $Pile[0])))));
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_mots_objet @ plugins/auto/metasplus_v2/inclure/metasplus/dist.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_images_objethtml_2b86667419653cd84b50e539bba36c01(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$command['where'] = 
			...
			array('NOT', 
			array('=', 'documents.fichier', sql_quote(replace(replace(table_valeur($Pile["vars"], (string)'logos_auto/0/url', null),constant('_NOM_PERMANENTS_ACCESSIBLES')),'\\.\\./'), '', 'text NOT NULL'))));
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('plugins/auto/metasplus_v2/inclure/metasplus/dist.html','html_2b86667419653cd84b50e539bba36c01','_images_objet',134,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
vide($Pile['vars'][$_zzz=(string)'logos_auto'] = filtre_push(table_valeur($Pile["vars"], (string)'logos_auto', null),array('url' => interdire_scripts(get_spip_doc($Pile[$SP]['fichier'])), 'alt' => interdire_scripts(couper(((($a = supprimer_numero(typo($Pile[$SP]['titre']), "TYPO", $connect, $Pile[0])) OR (is_string($a) AND strlen($a))) ? $a : interdire_scripts(propre($Pile[$SP]['descriptif'], $connect, $Pile[0]))),'80'))))));
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_images_objet @ plugins/auto/metasplus_v2/inclure/metasplus/dist.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_logo_objethtml_2b86667419653cd84b50e539bba36c01(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"CONDITION",
		$command,
		array('plugins/auto/metasplus_v2/inclure/metasplus/dist.html','html_2b86667419653cd84b50e539bba36c01','_logo_objet',126,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
BOUCLE_images_objethtml_2b86667419653cd84b50e539bba36c01($Cache, $Pile, $doublons, $Numrows, $SP) .
'
');
	}
	$iter->free();
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_logo_objet @ plugins/auto/metasplus_v2/inclure/metasplus/dist.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_objethtml_2b86667419653cd84b50e539bba36c01(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"CONDITION",
		$command,
		array('plugins/auto/metasplus_v2/inclure/metasplus/dist.html','html_2b86667419653cd84b50e539bba36c01','_objet',87,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
(($t1 = BOUCLE_auteurs_objethtml_2b86667419653cd84b50e539bba36c01($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		$t1 :
		((	'
' .
...
BOUCLE_mots_objethtml_2b86667419653cd84b50e539bba36c01($Cache, $Pile, $doublons, $Numrows, $SP) .
'


' .
BOUCLE_logo_objethtml_2b86667419653cd84b50e539bba36c01($Cache, $Pile, $doublons, $Numrows, $SP) .
'
');
	}
	$iter->free();
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_objet @ plugins/auto/metasplus_v2/inclure/metasplus/dist.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}

//
// Fonction principale du squelette plugins/auto/metasplus_v2/inclure/metasplus/dist.html
// Temps de compilation total: 58.107 ms
//

function html_2b86667419653cd84b50e539bba36c01($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
...
' .
(($t1 = BOUCLE_objethtml_2b86667419653cd84b50e539bba36c01($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		$t1 :
		((	'
...
'<'.'?php echo recuperer_fond( ' . argumenter_squelette('inclure/metasplus/inc-dist') . ', array(\'titre\' => ' . argumenter_squelette(interdire_scripts(entites_html(sinon(table_valeur(@$Pile[0], (string)'titre', null), table_valeur($Pile["vars"], (string)'titre_auto', null)),true))) . ',
	...
	\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . '), array("compil"=>array(\'plugins/auto/metasplus_v2/inclure/metasplus/dist.html\',\'html_2b86667419653cd84b50e539bba36c01\',\'\',177,$GLOBALS[\'spip_lang\'])), _request("connect"));
?'.'>

' .
'<' . '?php header("X-Spip-Filtre: '.'trim' . '"); ?'.'>');

	return analyse_resultat_skel('html_2b86667419653cd84b50e539bba36c01', $Cache, $page, 'plugins/auto/metasplus_v2/inclure/metasplus/dist.html');
}
?>