...
/*
 * Squelette : squelettes/recherche.html
 * Date :      Mon, 03 Feb 2020 15:41:40 GMT
 * Compile :   Fri, 07 Feb 2020 20:56:11 GMT
 * Boucles :   _articles, _rubriques, _mots, _breves, _messages, _sites
 */ 

function BOUCLE_articleshtml_f5802a09a8bc2d2e89d127b329e5d0dc(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('articles.statut','publie,prop,prepa/auteur','publie',''), $rech_where?$rech_where:'');
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/recherche.html','html_f5802a09a8bc2d2e89d127b329e5d0dc','_articles',27,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'</a></li>
	...
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_articles @ squelettes/recherche.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_rubriqueshtml_f5802a09a8bc2d2e89d127b329e5d0dc(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('rubriques.statut','!','publie',''), $rech_where?$rech_where:'');
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/recherche.html','html_f5802a09a8bc2d2e89d127b329e5d0dc','_rubriques',41,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'</a></li>
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_rubriques @ squelettes/recherche.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_motshtml_f5802a09a8bc2d2e89d127b329e5d0dc(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/recherche.html','html_f5802a09a8bc2d2e89d127b329e5d0dc','_mots',55,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'</a></li>
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_mots @ squelettes/recherche.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_breveshtml_f5802a09a8bc2d2e89d127b329e5d0dc(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('breves.statut','publie,prop','publie',''), $rech_where?$rech_where:'');
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/recherche.html','html_f5802a09a8bc2d2e89d127b329e5d0dc','_breves',69,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'</a></li>
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_breves @ squelettes/recherche.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_messageshtml_f5802a09a8bc2d2e89d127b329e5d0dc(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('forum.statut','publie,prop','publie',''), 
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/recherche.html','html_f5802a09a8bc2d2e89d127b329e5d0dc','_messages',83,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_messages @ squelettes/recherche.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}


function BOUCLE_siteshtml_f5802a09a8bc2d2e89d127b329e5d0dc(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
...
quete_condition_statut('syndic.statut','publie,prop','publie',''), $rech_where?$rech_where:'');
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('squelettes/recherche.html','html_f5802a09a8bc2d2e89d127b329e5d0dc','_sites',97,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
...
'</a></li>
	...
	if (defined("_BOUCLE_PROFILER")
	AND 1000*($timer = (time()+(float)microtime())-$timer) > _BOUCLE_PROFILER)
		spip_log(intval(1000*$timer)."ms BOUCLE_sites @ squelettes/recherche.html","profiler"._LOG_AVERTISSEMENT);
	return $t0;
}

//
// Fonction principale du squelette squelettes/recherche.html
// Temps de compilation total: 23.246 ms
//

function html_f5802a09a8bc2d2e89d127b329e5d0dc($Cache, $Pile, $doublons = array(), $Numrows = array(), $SP = 0) {
...
'<'.'?php echo recuperer_fond( ' . argumenter_squelette('inclure/head') . ', array(\'robots\' => ' . argumenter_squelette('none') . ',
	\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . '), array("compil"=>array(\'squelettes/recherche.html\',\'html_f5802a09a8bc2d2e89d127b329e5d0dc\',\'\',10,$GLOBALS[\'spip_lang\'])), _request("connect"));
?'.'>
</head>
...
'<'.'?php echo recuperer_fond( ' . argumenter_squelette('inclure/header') . ', array(\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . '), array("compil"=>array(\'squelettes/recherche.html\',\'html_f5802a09a8bc2d2e89d127b329e5d0dc\',\'\',14,$GLOBALS[\'spip_lang\'])), _request("connect"));
?'.'>
	' .

'<'.'?php echo recuperer_fond( ' . argumenter_squelette('inclure/nav') . ', array_merge('.var_export($Pile[0],1).',array(\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . ')), array("compil"=>array(\'squelettes/recherche.html\',\'html_f5802a09a8bc2d2e89d127b329e5d0dc\',\'\',15,$GLOBALS[\'spip_lang\'])), _request("connect"));
?'.'>
	<div class="main">
...
'
                        ...
                        ' .
(($t1 = BOUCLE_articleshtml_f5802a09a8bc2d2e89d127b329e5d0dc($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
...
'
                        ...
                        ' .
(($t1 = BOUCLE_rubriqueshtml_f5802a09a8bc2d2e89d127b329e5d0dc($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
...
'
                        ...
                        ' .
(($t1 = BOUCLE_motshtml_f5802a09a8bc2d2e89d127b329e5d0dc($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
...
'
                        ...
                        ' .
(($t1 = BOUCLE_breveshtml_f5802a09a8bc2d2e89d127b329e5d0dc($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
...
'
                        ...
                        ' .
(($t1 = BOUCLE_messageshtml_f5802a09a8bc2d2e89d127b329e5d0dc($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
...
'
                        ...
                        ' .
(($t1 = BOUCLE_siteshtml_f5802a09a8bc2d2e89d127b329e5d0dc($Cache, $Pile, $doublons, $Numrows, $SP))!=='' ?
		((	'
...
'<'.'?php echo recuperer_fond( ' . argumenter_squelette('inclure/navsub') . ', array(\'id_rubrique\' => ' . argumenter_squelette(@$Pile[0]['id_rubrique']) . ',
	\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . '), array("compil"=>array(\'squelettes/recherche.html\',\'html_f5802a09a8bc2d2e89d127b329e5d0dc\',\'\',112,$GLOBALS[\'spip_lang\'])), _request("connect"));
?'.'>
		</div><!--.aside-->
	</div><!--.main-->
	' .
...
'<'.'?php echo recuperer_fond( ' . argumenter_squelette('inclure/footer') . ', array_merge('.var_export($Pile[0],1).',array(\'self\' => ' . argumenter_squelette(self()) . ',
	\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . ')), array("compil"=>array(\'squelettes/recherche.html\',\'html_f5802a09a8bc2d2e89d127b329e5d0dc\',\'\',115,$GLOBALS[\'spip_lang\'])), _request("connect"));
?'.'>
    </div><!--.page-->
</body>
</html>');

	return analyse_resultat_skel('html_f5802a09a8bc2d2e89d127b329e5d0dc', $Cache, $page, 'squelettes/recherche.html');
}
?>