...
function BOUCLE_depot_existehtml_eb700f30c56a1c4003ca29f82287d3a4(&$Cache, &$Pile, &$doublons, &$Numrows, $SP) {
	...
	$iter = IterFactory::create(
		"SQL",
		$command,
		array('../plugins-dist/svp/formulaires/charger_plugin.html','html_eb700f30c56a1c4003ca29f82287d3a4','_depot_existe',8,$GLOBALS['spip_lang'])
	);
	if (!$iter->err()) {
	$SP++;
	// RESULTATS
...
'<'.'?php echo recuperer_fond( ' . argumenter_squelette('formulaires/inc-select_categorie') . ', array_merge('.var_export($Pile[0],1).',array(\'categorie\' => ' . argumenter_squelette(interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'categorie', null),true))) . ',
	\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . ')), array("compil"=>array(\'../plugins-dist/svp/formulaires/charger_plugin.html\',\'html_eb700f30c56a1c4003ca29f82287d3a4\',\'\',31,$GLOBALS[\'spip_lang\'])), _request("connect"));
?'.'>
					</div>
...
'<'.'?php echo recuperer_fond( ' . argumenter_squelette('formulaires/inc-select_depot') . ', array_merge('.var_export($Pile[0],1).',array(\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . ')), array("compil"=>array(\'../plugins-dist/svp/formulaires/charger_plugin.html\',\'html_eb700f30c56a1c4003ca29f82287d3a4\',\'\',47,$GLOBALS[\'spip_lang\'])), _request("connect"));
?'.'>
					</div>
...
'<'.'?php echo recuperer_fond( ' . argumenter_squelette('formulaires/inc-plugins_trouves') . ', array_merge('.var_export($Pile[0],1).',array(\'plugins\' => ' . argumenter_squelette(interdire_scripts(table_valeur(table_valeur(@$Pile[0], (string)'message_ok', null),'plugins'))) . ',
	\'lang\' => ' . argumenter_squelette($GLOBALS["spip_lang"]) . ')), array("compil"=>array(\'../plugins-dist/svp/formulaires/charger_plugin.html\',\'html_eb700f30c56a1c4003ca29f82287d3a4\',\'\',67,$GLOBALS[\'spip_lang\'])), _request("connect"));
?'.'>
			</fieldset>
	...
	(($t2 = strval(interdire_scripts(((entites_html(table_valeur(@$Pile[0], (string)'ids_paquet', null),true)) ?'' :' '))))!=='' ?
			...
			'') .
	'>
				<input type="submit" name="telecharger" class="submit" value="' .
...
'
	' .
interdire_scripts(table_valeur(@$Pile[0], (string)'_actions_realisees', null)) .
'
